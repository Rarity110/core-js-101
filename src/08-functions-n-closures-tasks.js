...
function getPolynom(...args) {
  ...
  const len = arrArgs.length - 1;
  return (x) => arrArgs.reduce((a, b, i) => a + b * x ** (len - i), 0);
}
...
function retry(func, attempts) {
  return function repeatFunc(count = attempts) {
    try {
      return func();
    } catch (error) {
...
function logger(func, logFunc) {
  return (...args) => {
    const arrArgs = args.map((el) => JSON.stringify(el)).join(',');
